...
def electre(x_matrix: Matrix, w: Vector) -> int:
    ...
    for i in range(m):
        for j in range(m):
            ...
            diff: Vector = (x_matrix[i] - x_matrix[j]) / (x_matrix.max(axis=0) - x_matrix.min(axis=0) + 1e-12)
            discordance[i, j] = diff.max()

    outrank: BoolVector = (concordance >= _CONC_THRESHOLD) & (discordance <= _DIS_THRESHOLD)
